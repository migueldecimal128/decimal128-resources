// =============================================================================
//  operator.h 
//  Defines the names for operations in dfp libraries. 
// =============================================================================
//  Original Author : Liang-Kai Wang, 5/2007
//  Maintainer:      Mike Anderson 1/2009 
// =============================================================================
//  This file defines the names for all operations in each DFP library. 
//  Currently supported libraries are:
//  DECNUMBER_ARBITRARY (IBM DecNumber arbitrary-precision types) 
//  DECNUMBER_FIXED (IBM DecFloats fixed-precision types)
//  BID (Intel Decimal Floating-Point Math Library)
//  NATIVE_GCC_BID (gcc native dfp types)
...
// Id: $Id: operator.h,v 1.22 2009-04-21 06:09:16 mjanders Exp $
// Date: $Date: 2009-04-21 06:09:16 $
...
// Convert to binary double to do power operation
/*#define POWER(Z, X, Y, C) { \
  double base, exponent, result; \
  char base[255]; \
  char exponent[255]; \
  base = atof(decNumberToString(X, base)); \
  exponent = atof(decNumberToString(Y, exponent)); \
...
#define LOG10(Z, X, C) {\
 decNumber dnX, dnZ;\
 decQuadToNumber(X, &dnX);\
 decNumberLog10(&dnZ, &dnX, C);\
 decQuadFromNumber(Z, &dnZ, C); \
 }
...
#define CONTEXTDEFAULT(X, Y) X = RNE
...
// Convert to binary double to do log operation
#define LOG10(Z, X, C) { \
  double input, result; \
...
#define LOG10(Z, X, C) { \
  double base, result; \
  __bid128_to_binary64(&base, X, C); \
  result = log(base); \
  __binary64_to_bid128(Z, &result, C); \
}
...
#define POWER(Z, X, Y, C) { \
  double base, exponent, result; \
  __bid128_to_binary64(&base, X, C); \
  __bid128_to_binary64(&exponent, Y, C); \
  result = pow(base, exponent); \
  __binary64_to_bid128(Z, &result, C); \
}
...
#define ABS(Z, X, C) __bid64_abs((BID_UINT64*)Z, (BID_UINT64*)X) 
#define ADD(Z, X, Y, C) *Z = *X + *Y 
#define EQ(Z, X, Y, C)  __bid64_quiet_equal(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define GT(Z, X, Y, C)  __bid64_quiet_greater(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define LT(Z, X, Y, C)  __bid64_quiet_less(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define GE(Z, X, Y, C)  __bid64_quiet_greater_equal(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define LE(Z, X, Y, C)  __bid64_quiet_less_equal(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define NE(Z, X, Y, C)  __bid64_quiet_not_equal(Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define DIVIDE(Z, X, Y, C) *Z = (*X) / (*Y) 
// Convert to binary double to do log operation
#define LOG10(Z, X, C) { \
  double input, result; \
  __bid64_to_binary64(&input, (BID_UINT64*)X, C, &flags); \
  result = log(input); \
  __binary64_to_bid64((BID_UINT64*)Z, &result, C, &flags); \
}  
#define MAX(Z, X, Y, C) __bid64_maxnum((BID_UINT64*)Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags)
#define MIN(Z, X, Y, C) __bid64_minnum((BID_UINT64*)Z, (BID_UINT64*)X, (BID_UINT64*)Y, &flags) 
#define MINUS(Z, X, C) __bid64_negate((BID_UINT64*)Z, (BID_UINT64*)X)
#define MULTIPLY(Z, X, Y, C) *Z = (*X) * (*Y)
// Convert to binary double to do power operation
#define POWER(Z, X, Y, C) { \
  double base, exponent, result; \
  __bid64_to_binary64(&base, (BID_UINT64*)X, C, &flags); \
  __bid64_to_binary64(&exponent, (BID_UINT64 *)Y, C, &flags); \
  result = pow(base, exponent); \
  __binary64_to_bid64((BID_UINT64*)Z, &result, C, &flags); \
}  
...
#define COPY(Z, X) __bid64_copy((BID_UINT64*)Z, (BID_UINT64*)X) 
#define ZERO(Z) *Z = 0
...
/*
  ...
  $Date: 2009-04-21 06:09:16 $ - The date of the last commit.
  $Name:  $ - The tag name
  $Log: operator.h,v $
  Revision 1.22  2009-04-21 06:09:16  mjanders
  Running simulations. Updating libraries. Compiling without flags

  Revision 1.21  2009-04-19 21:40:08  mjanders
  Updated decnumber and BID libraries

  Revision 1.20  2009-04-14 02:29:58  mjanders
  Bug showed up with 03

  Revision 1.19  2009-04-13 00:41:38  mjanders
  Removed debug output and fixed bug with 128-bit

  Revision 1.18  2009-04-12 05:56:19  mjanders
  Added 128-bit mode

  Revision 1.17  2009-01-08 20:38:58  mjanders
  Added documentation

  Revision 1.16  2009/01/06 18:39:12  mjanders
  Added common header

  Revision 1.15  2009/01/05 23:01:01  mjanders
  Fixed bugs in DECNUMBER_ARBITRARY rounding function calls

  Revision 1.14  2009/01/03 01:12:49  mjanders
  Fixed typos

  Revision 1.13  2009/01/03 00:04:33  mjanders
  Added round modes for native_gcc_bid

  Revision 1.12  2009/01/02 23:53:27  mjanders
  Added native_gcc_bid mode and tested

  Revision 1.11  2008/11/25 06:16:58  mjanders
  Finished adding BID

  Revision 1.10  2008/11/23 08:14:17  mjanders
  Moved around the include files to make more sense. All include files for the libraries come in operator.h

  Revision 1.9  2008/11/21 04:35:22  mjanders
  Added BID and native types. Fixed some issues with not enough arguments etc.

  Revision 1.8  2008/11/19 06:12:45  mjanders
  Added support for native types, though the library won't work yet

  Revision 1.7  2008/05/15 04:54:34  mjanders
  Switched to new way of comparing numbers

  Revision 1.6  2008/05/14 22:15:03  mjanders
  Adding round mode functions and fixing bugs for BID

  Revision 1.5  2008/05/13 19:01:20  mjanders
  Added BID functions. Not tested yet.

  Revision 1.4  2008/05/01 20:50:54  stsen
  updated for first changes to intel library

  Revision 1.3  2008/04/24 22:01:25  mjanders
  Fixed bug in TOINGEGRAL for the counting mode and added COUNTING mode
  to the Makefile.

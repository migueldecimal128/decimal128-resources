...
// Id: $Id: credit_card.c,v 1.13 2009-05-18 03:07:53 mjanders Exp $
...
//////////////////////////////////////////////////////
extern dec_t ONE, MTWO, ETWO, HUNDRED, YEAR, CENT;
extern int counter;
...
{
  cc_acct_t *this_acct;
  char p_debt[LENGTH_T], c_debt[LENGTH_T], b_debt[LENGTH_T];
  char input_str[STR_MAX_LENGTH];
...
{
  dec_t comp;
  dec_t *lowest, *middle, *largest;
  dec_t *lowest_v, *middle_v, *largest_v;
  int array[3];
  dec_t temp;
...
int credit_card_acct_debt(htab *cc_table, char *acct_no, int currency, 
			  ...
			  int type, dec_t fund)
{
  dec_t converted_fund, r_plus, result;
  dec_t converted_rate;
  dec_t transaction;
  dec_t temp;
  cc_acct_t *this_acct;
			  ...
			  FILE *fout)
{
  ...
  {
    mysprintf(input_str, "[credit_card_acct_open] cannot add %s's account\n",
            acct_no);
...
{
  char str1[LENGTH_T], str2[LENGTH_T], unit[LENGTH_T];
  dec_t temp;
  cc_acct_t *this_acct;
  if (hfind(cc_table, acct_no, mystrlen(acct_no))==FALSE)
  {

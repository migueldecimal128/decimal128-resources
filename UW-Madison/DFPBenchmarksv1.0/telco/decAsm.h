...
#define ABS32(Z, X, C)       ABS(&Z, &X, C)
#define ABS64(Z, X, C)       ABS(&Z, &X, C)
#define ADD32(Z, X, Y, C)    ADD(&Z, &X, &Y, C)
#define ADD64(Z, X, Y, C)    ADD(&Z, &X, &Y, C)
#define COMPARE32(Z, X, Y, C)   COMPARE(&Z, &X, &Y, C)
...
#define COMPARETOTAL32(Z, X, Y, C) {decimal32 dummy; decimal32FromNumber(&dumm\
y, &X, C); decimal32ToNumber(&dummy, &X); decimal32ToNumber(&dummy, &X); COMPARETOTAL(&Z, &X, &Y, C);}
#define COMPARETOTAL64(Z, X, Y, C) {decimal64 dummy; decimal64FromNumber(&dumm\
y, &X, C); decimal64ToNumber(&dummy, &X); decimal64ToNumber(&dummy, &X); COMPARETOTAL(&Z, &X, &Y, C);}
#define DIVIDE32(Z, X, Y, C)  DIVIDE(&Z, &X, &Y, C)
#define DIVIDE64(Z, X, Y, C)  DIVIDE(&Z, &X, &Y, C)
#define DIVIDEINTEGER32(Z, X, Y, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); decimal32ToNumber(&dummy, &X); DIVIDEINTEGER(&Z, &X, &Y, C);}
#define DIVIDEINTEGER64(Z, X, Y, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X); decimal64ToNumber(&dummy, &X); DIVIDEINTEGER(&Z, &X, &Y, C);}
#define EXP32(Z, X, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); EXP(&Z, &X, C);}
#define EXP64(Z, X, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X); EXP(&Z, &X, C);}
#define LN32(Z, X, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); de\
cimal32ToNumber(&dummy, &X); LN(&Z, &X, C);}
#define LN64(Z, X, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); de\
cimal64ToNumber(&dummy, &X); LN(&Z, &X, C);}
#define LOG1032(Z, X, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); LOG10(&Z, &X, C);}
#define LOG1064(Z, X, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X); LOG10(&Z, &X, C);}
#define MAX32(Z, X, Y, C) MAX(&Z, &X, &Y, C)
#define MAX64(Z, X, Y, C) MAX(&Z, &X, &Y, C)
#define MIN32(Z, X, Y, C) MIN(&Z, &X, &Y, C)
#define MIN64(Z, X, Y, C) MIN(&Z, &X, &Y, C)
#define MINUS32(Z, X, C)  MINUS(&Z, &X, C)
#define MINUS64(Z, X, C)  MINUS(&Z, &X, C)
#define MULTIPLY32(Z, X, Y, C) MULTIPLY(&Z, &X, &Y, C)
#define MULTIPLY64(Z, X, Y, C) MULTIPLY(&Z, &X, &Y, C)
#define NORMALIZE32(Z, X, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); NORMALIZE(&Z, &X, C);}
#define NORMALIZE64(Z, X, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X);NORMALIZE(&Z, &X, C);}
#define PLUS32(Z, X, C) PLUS(&Z, &X, C)
#define PLUS64(Z, X, C) PLUS(&Z, &X, C)
#define POWER32(Z, X, Y, C)  {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); decimal32ToNumber(&dummy, &X); POWER(&Z, &X, &Y, C);}
#define POWER64(Z, X, Y, C)   {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X); decimal64ToNumber(&dummy, &X); POWER(&Z, &X, &Y, C);}
#define QUANTIZE32(Z, X, Y, C)  QUANTIZE(&Z, &X, &Y, C) 
#define QUANTIZE64(Z, X, Y, C)  QUANTIZE(&Z, &X, &Y, C) 
#define RESCALE32(Z, X, Y, C) RESCALE(&Z, &X, &Y, C)
#define RESCALE64(Z, X, Y, C) RESCALE(&Z, &X, &Y, C)
#define REMAINDER32(Z, X, Y, C) {decimal32 dummy; decimal32FromNumber(&dummy, &X, C); decimal32ToNumber(&dummy, &X); decimal32ToNumber(&dummy, &X); REMAINDER(&Z, &X, &Y, C);}
#define REMAINDER64(Z, X, Y, C) {decimal64 dummy; decimal64FromNumber(&dummy, &X, C); decimal64ToNumber(&dummy, &X); decimal64ToNumber(&dummy, &X); REMAINDER(&Z, &X, &Y, C);}
#define SAMEQUANTUM32(Z, X, Y) SAMEQUANTUM(&Z, &X, &Y)
#define SAMEQUANTUM64(Z, X, Y) SAMEQUANTUM(&Z, &X, &Y)
#define SQUAREROOT32(Z, X, C) SQUAREROOT(&Z, &X, C)
#define SQUAREROOT64(Z, X, C) SQUAREROOT(&Z, &X, C)
#define SUBTRACT32(Z, X, Y, C) SUBTRACT(&Z, &X, &Y, C)
#define SUBTRACT64(Z, X, Y, C) SUBTRACT(&Z, &X, &Y, C)
#define TOINTEGRAL32(Z, X, C) TOINTEGRAL(&Z, &X, C)
#define TOINTEGRAL64(Z, X, C) TOINTEGRAL(&Z, &X, C)
#define COPY32(Z, X) COPY(&Z, &X)
#define COPY64(Z, X) COPY(&Z, &X)
#define ZERO32(Z) ZERO(&Z)
#define ZERO64(Z) ZERO(&Z)
#define FROMSTRING32(Z, R, C)    FROMSTRING(&Z, R, C)
#define FROMSTRING64(Z, R, C)    FROMSTRING(&Z, R, C)
#define FROMINTEGER32(Z, R, C)     FROMINTEGER(&Z, R, C)
#define FROMINTEGER64(Z, R, C)     FROMINTEGER(&Z, R, C)
#define TOSTRING32(Z, R)      TOSTRING(&Z, R)
#define TOSTRING64(Z, R)      TOSTRING(&Z, R)
#define TOENGSTRING32(Z, R)   TOENGSTRING(&Z, R)
#define TOENGSTRING64(Z, R)   TOENGSTRING(&Z, R)
#define ISNEGATIVE32(Z)       ISNEGATIVE(&Z)
#define ISNEGATIVE64(Z)       ISNEGATIVE(&Z)
#define ISZERO32(Z)     ISZERO(&Z)
#define ISZERO64(Z)     ISZERO(&Z)
#define ISPOSITIVE32(Z) ((ISZERO32(Z)==0)&&(ISNEGATIVE32(Z)==0))
#define ISPOSITIVE64(Z) ((ISZERO64(Z)==0)&&(ISNEGATIVE64(Z)==0))
#define ISINFINITE32(Z) ISINFINITE(&Z)
#define ISINFINITE64(Z) ISINFINITE(&Z)
#define ISNAN32(Z)  ISNAN(&Z)
#define ISNAN64(Z)  ISNAN(&Z)
#define ISNORMAL32(ZM)  (ISNAN32(ZM)==0)&&(ISINFINITE32(ZM)==0)
#define ISNORMAL64(ZM)  (ISNAN64(ZM)==0)&&(ISINFINITE64(ZM)==0)
#define TRIM32(Z)  TRIM(&Z)

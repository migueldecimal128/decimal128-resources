...
#define RESCALE(Z, X, Y, C){\
 ...
 decimal64FromNumber(&x64, X, C);\
 decimal64FromNumber(&y64, Y, C);\
 rescale_check(&x64, &y64, X, Y);\
 rescale_toint_total++;\
 (C)->status = (C)->status & (~DEC_Rounded);\
 decNumberRescale(Z, X, Y, C);\
 if ((C)->status & DEC_Rounded)\
   rescale_toint_rounded++;\
}
#else // COUNTING (DECNUMBER_ARBITRARY)
...
#define TOINTEGRAL(Z, X, C){\
 ...
 decimal64FromNumber(&y64, &zero_local, C);\
   toint_check(&x64, &y64, X, &zero_local);\
   rescale_toint_total++;\
...
#define GT(Z, X, Y, C)  {\                         decDouble tmp;\
                        decDoubleCompare(&tmp, X, Y, C);\
                        *Z = ISPOSITIVE(&tmp);\
}
#define LT(Z, X, Y, C)  {\
...
/*
  ...
  $Name:  $ - The tag name
  $Log: operator.h,v $
  Revision 1.11  2009-04-19 21:51:57  mjanders
  Updated decnumber and Intel libraries

  Revision 1.10  2009-04-16 05:24:49  mjanders
  Fixed things while doing simulations

  Revision 1.9  2009-04-14 19:34:09  mjanders
  O3

  Revision 1.8  2009-03-20 20:15:28  mjanders
  Added 128-bit mode

  Revision 1.7  2009-01-03 01:13:49  mjanders
  Added rounding for native_gcc_bid

  Revision 1.6  2008/11/23 07:57:55  mjanders
  Added header file includes (where they belong) and changed log and power over to binary

  Revision 1.5  2008/11/19 06:22:09  mjanders
  Fixed global flags and fixed FROMINTEGER

  Revision 1.4  2008/09/24 07:36:05  mjanders
  Added native gcc types

  Revision 1.3  2008/08/08 07:42:19  mjanders
  Added support for BID

  Revision 1.2  2008/04/24 22:09:52  mjanders
  Fixed bug in TOINTEGRAL.

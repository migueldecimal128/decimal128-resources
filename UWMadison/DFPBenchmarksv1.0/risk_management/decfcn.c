...
//This function call is to emulate the time spending on fprintf. This is 
//mainly to be used with gprof
int decfprintf(FILE *stream, int argc, const char *format, ...){
  ...
  va_list ap;
  va_start(ap, format);
  argv = calloc(argc, sizeof(dec_t));
  fprintf(stream, "%s = (", format);
  for (x = 0; x<argc; x++){
    ptr = va_arg(ap, dec_t *);
    TOSTRING64(*ptr, str);
    if (x<argc-1)
      retval = fprintf(stream, "%s, ", str);
    else
...
//The following is used with OS=LINUX
#ifdef COUNTING
...
void decimal_printout(FILE *fd){
  fprintf(fd, "Rounded Result: Add = %d, Rescale/toInt = %d, Multiply = %d, Divide = %d\n",
          add_rounded, rescale_toint_rounded, mul_rounded, div_rounded);
  fprintf(fd, "Total Add = %d, Total Min/max = %d, Total Rescale/toInt = %d, Multiply = %d, Divide = %d\n",
          add_total, minmax_total, rescale_toint_total, mul_total, div_total);
...
//Function to extract the exponent from an operand
...
//Function call to check the operands to decimal add and subtract
...
int rescale_check(decimal64 *a, decimal64 *b, decNumber *an, 
		      decNumber *bn){
  unsigned char msd_a[2], msd_b[2];
  int test;
  int expA, expB;

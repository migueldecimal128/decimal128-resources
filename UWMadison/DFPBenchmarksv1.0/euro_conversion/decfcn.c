...
_IDEC_flags flags;      // working flags 
#endif
...
//One or more operands are zero to DFP Add/Sub
...
int extract_exp(int test, unsigned char *msd_a){
  int exp = 0;
  //test is the signal for the two MSB of the biased exponent
  if (test!=0){
...
int add_check(decimal64 *a, decimal64 *b, decNumber *an, decNumber *bn){
  ...
  int test;
  int expA, expB;
  //If any of the op is not normal number, skip it
  if (decNumberIsNaN(an) || decNumberIsNaN(bn) ||
      decNumberIsInfinite(an) || decNumberIsInfinite(bn)){
    return INVALID;
  }
  //Record the first two bytes from the input op

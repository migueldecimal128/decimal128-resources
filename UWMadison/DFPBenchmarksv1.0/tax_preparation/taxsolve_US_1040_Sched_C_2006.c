...
/* 02111-1307 USA							*/
/* 									*/
/* S.Jenkinss 1-26-2004	  sarah.jenkins@simnews.com			*/
/* Updated 2-10-06	Robert Heller  heller@deepsoft.com		*/
/************************************************************************/

float thisversion=4.0;
...
int main ( int argc, char *argv[] )
{
 ...
 time_t now;
 float L16b=0.0, L20b=0.0, L24b=0.0, L24c=0.0, L24d=0.0;
 int L32;

 printf("US 1040 Schedule C, 2006 - v%3.2f\n", thisversion);
  ...
  if (k==1)
   {
    ...
    if (j<0) strcat(outfname,".out"); else strcpy(&(outfname[j]),".out");
 ...
 */
 ...
 GetLine( "L4", &L[4] );	/* Cost of Goods and Services */

 GetLine( "L6", &L[6] );	/* Other income, including fed & state fuel credit (pg C-3) */

 GetLine( "L8", &L[8] );	/* Advertising */

 GetLine( "L9", &L[9] );	/* Car & truck expenses (pg C-3) */

 GetLine( "L10", &L[10] );	/* Communications & fees */

 GetLine( "L11", &L[11] );	/* Contract labor (pg C-4) */

 GetLine( "L12", &L[12] );	/* Depletion */

 GetLine( "L13", &L[13] );	/* Depreciation & Sec 179 exp ded (pg C-4) */
 ...
 GetLine( "L20a", &L[20] );	/* Vehicles Rent or lease (C-5)  */

 GetLine( "L20b", &L20b );	/* Rent or lease (C-5) Other business property */

 GetLine( "L21", &L[21] );	/* Repairs & maintenance */

 GetLine( "L22", &L[22] );	/* Supplies (no in Part III) */

 GetLine( "L23", &L[23] );	/* Taxes & licenses */

 GetLine( "L24a", &L[24] );	/* Travel */

 GetLine( "L24b", &L24b );	/* Meals & entertainment */

 GetLine( "L24c", &L24c );	/* Nondeductible amount in L24b */

 GetLine( "L25", &L[25] );	/* Utilities */

 GetLine( "L26", &L[26] );	/* Wages (less employment credits) */

 GetLine( "L27", &L[27] );	/* List other expenses here from line 48 pg 2 */

 GetLine( "L30", &L[30] );	/* Expenses for business use of home (form 8829) */

 get_parameter( infile, 's', word, "L32a" ); /* Yes or No, All investment is at risk (pg C-6) */
 ...
 fprintf(outfile,"L24c = %6.2f\n", L24c);
 L24d = L24b - L24c;
 fprintf(outfile,"L24d = %6.2f\n", L24d);
 showline(25);
 showline(26);
 showline(27);
 L[28] = L[8] + L[9] + L[10] + L[11] + L[12] + L[13] + L[14] + L[15] + L[16] + L16b + L[17] + L[18] 
 	 + L[19] + L[20] + L20b + L[21] + L[22] + L[23] + L[24] + L24d + L[25] + L[26] + L[27];
 ...
 if (L32)  { fprintf(outfile,"Mark box L32a\n"); }
 else  	   { fprintf(outfile,"Mark box L32b\n"); }

 if (L[31]<0.0)
  {
   if (L32) fprintf(outfile,"Enter %2.2f on Form 1040 line 12, Sched SE line 2. Estates/trusts on 1040 line 3.\n", L[31]);
   else fprintf(outfile,"You MUST attach Form 6198.\n");
  }
